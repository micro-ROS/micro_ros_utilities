...
static const char * basic_type_names[] = {"none", "float", "double", "long double", "char", "wchar", "bool", "octec", "uint8", "int8", "uint16", "int16", "uint32", "int32", "uint64", "int64", "string", "wstring", "ROS2 type" };
static const size_t basic_types_size[] = {0, sizeof(float), sizeof(double), sizeof(double), sizeof(char), sizeof(char), sizeof(bool), sizeof(char), sizeof(uint8_t), sizeof(uint8_t), sizeof(uint16_t), sizeof(uint16_t), sizeof(uint32_t), sizeof(uint32_t), sizeof(uint64_t), sizeof(uint64_t), sizeof(char), sizeof(char), 0};
...
static micro_ros_utilities_memory_conf_t get_configuration(const micro_ros_utilities_memory_conf_t conf)
{
    micro_ros_utilities_memory_conf_t ret = memory_conf_default;
    ret.max_basic_type_sequence = (conf.max_basic_type_sequence == 0) ? memory_conf_default.max_basic_type_sequence : conf.max_basic_type_sequence;
    ret.max_ros2_type_sequence = (conf.max_ros2_type_sequence == 0) ? memory_conf_default.max_ros2_type_sequence : conf.max_ros2_type_sequence;
    ret.max_string_capacity = (conf.max_string_capacity == 0) ? memory_conf_default.max_string_capacity : conf.max_string_capacity;
...
void print_type_info(const rosidl_typesupport_introspection_c__MessageMembers* members, rosidl_runtime_c__String *out, rosidl_runtime_c__String indent_buffer, uint32_t level)
{
	...
	if (level == 0)
	{
		sprintf(buffer,"%sIntrospection for %s/%s - %d members, %ld B\n",
			indent_buffer.data,
    ...
    handle_message_memory_operation_t operation)
{
    ...
    for (size_t i = 0; i < members->member_count_; i++)
    {
        ...
        if (m.type_id_ == 16 || is_sequence)
        {
            ...
            if (m.type_id_ == 18)
            {
                const rosidl_message_type_support_t* introspection = get_message_typesupport_handle(m.members_, rosidl_typesupport_introspection_c__identifier);
                rec_members = (rosidl_typesupport_introspection_c__MessageMembers*)introspection->data;
            }

            size_t member_size = (m.type_id_ == 18) ? rec_members->size_of_ : basic_types_size[m.type_id_];
        ...
        {
            const rosidl_message_type_support_t* introspection = get_message_typesupport_handle(m.members_, rosidl_typesupport_introspection_c__identifier);
            rosidl_typesupport_introspection_c__MessageMembers* rec_members = (rosidl_typesupport_introspection_c__MessageMembers*)introspection->data;

            if ((m.type_id_ == 16 || m.type_id_ == 18) && is_sequence){
                generic_sequence_t * ptr = (generic_sequence_t*)((uint8_t * )ros_msg + m.offset_);
                for (size_t i = 0; i < sequence_size; i++)
                {
                    uint8_t * data = (operation == CALCULATE_SIZE_OPERATION) ? NULL : (uint8_t*)ptr->data + (i * rec_members->size_of_);
            ...
            {
                used_memory += handle_message_memory(rec_members, ((uint8_t*)ros_msg + m.offset_), conf, name_tree, operation);
            }
        }
...
size_t micro_ros_utilities_get_dynamic_size(const rosidl_message_type_support_t* type_support, const micro_ros_utilities_memory_conf_t conf)
{
    ...
    size_t size = handle_message_memory(members, NULL, actual_conf, name_tree, CALCULATE_SIZE_OPERATION);
...
bool micro_ros_utilities_create_message_memory(const rosidl_message_type_support_t* type_support, void* ros_msg, const micro_ros_utilities_memory_conf_t conf)
{
    ...
    size_t used_size = handle_message_memory(members, ros_msg, actual_conf, name_tree, CREATE_OPERATION);
    ...
    size_t buffer_len)
{
    ...
    memset(ros_msg, 0, members->size_of_);
    size_t used_size = handle_message_memory(members, ros_msg, actual_conf, name_tree, CREATE_PREALLOCATED_OPERATION);
...
bool micro_ros_utilities_destroy_message_memory(const rosidl_message_type_support_t* type_support, void* ros_msg, const micro_ros_utilities_memory_conf_t conf)
{
    ...
    size_t released_size = handle_message_memory(members, ros_msg, actual_conf, name_tree, DESTROY_OPERATION);
    memset(ros_msg, 0, members->size_of_);
